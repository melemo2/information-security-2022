...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    for idx in range(0, len(text), 8):
        result += sdes(text[idx:idx+8], key, MODE_ENCRYPT)
    return result

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    result = bitarray()
    for idx in range(0, len(ciphertext), 8):
        result += sdes(ciphertext[idx:idx+8], key, MODE_DECRYPT)
    return result
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    trans = iv[:8]
    for idx in range(0, len(text), 8):
        trans = sdes(trans ^ text[idx:idx+8], key, MODE_ENCRYPT)
        result += trans
    return result
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    trans = iv[:8]
    for idx in range(0, len(ciphertext), 8):
        result += (trans ^ sdes(ciphertext[idx:idx+8], key, MODE_DECRYPT))
        trans = ciphertext[idx:idx+8]
    return result
