...
def pass_wheels(input, reverse = False):
    ...
    if(reverse == False):
        for wheel in SETTINGS["WHEELS"]:
            input = wheel["wire"][ord(input) - ord('A')] # wheel 3개 통과
    else:
        for wheel in SETTINGS["WHEELS"][::-1]:
            input = chr(wheel["wire"].index(input) + ord('A')) # wheel 3개 통과
...
def rotate_wheels():
    ...
    wheel["wire"] = wheel["wire"][-1:] + wheel["wire"][:-1]

    for idx in range(len(SETTINGS["WHEELS"])):
        wheel = SETTINGS["WHEELS"][idx]
        if(idx < len(SETTINGS["WHEELS"]) & wheel["turn"] == SETTINGS["WHEEL_POS"][idx]): # wheel이 노치에 도달하면 다음 wheel 돌리기
            next_wheel = SETTINGS["WHEELS"][idx+1]
            SETTINGS["WHEEL_POS"][idx] = (SETTINGS["WHEEL_POS"][idx] + 1) % 26
            next_wheel["wire"] = next_wheel["wire"][-1:] + next_wheel["wire"][:-1]
    pass

# Enigma Exec Start
plaintext = "ZGVFZWKCPTZYUTBQ"#input("Plaintext to Encode: ")
ukw_select = "A"#input("Set Reflector (A, B, C): ")
wheel_select = "1 2 3"#input("Set Wheel Sequence L->R (I, II, III): ")
wheel_pos_select = "A B C"#input("Set Wheel Position L->R (A~Z): ")
