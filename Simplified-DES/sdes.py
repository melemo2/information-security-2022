...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    round_key = schedule_keys(key)[::mode] # 모드에 따라 키 순서 뒤집기
    # Place your own implementation of S-DES Here

    # 평문 재배치
    IP_result = bitarray()
    for idx in IP:
        IP_result.append(text[idx])

    text_left = IP_result[4:]
    text_right = IP_result[:4]

    round_right = round(text_right, round_key[0])
    cyper_left = text_left ^ round_right
 
    round_left = round(cyper_left, round_key[1])
    cyper_right = text_right ^ round_left

    text_cyper = (cyper_left + cyper_right)

    # 평문 재배치
    for idx in IP_1:
        result.append(text_cyper[idx])

    return result

#### DES Sample Program Start

plaintext = "10111101"#input("[*] Input Plaintext in Binary (8bits): ")
key = "0101010101"#input("[*] Input Key in Binary (10bits): ")
